...
##
## Nautilus
## SCRIPT: 00c_multi-AudVid-files_CONVERT_TOogg_ffmpeg-vn.sh
##
## PURPOSE: To convert selected media files --- such as '.mp3' or '.wav'
##          audio file(s) --- to '.ogg' audio file(s).
##          (Might also work on input movie files.)
##
## METHOD:  Uses 'ffmpeg -vn', in a loop on filenames.
##
##          The user should see the '.ogg' files popping up in the
##          'current' directory.
##
...
do
   ...
   ########################################################
   ## Check that the selected file is a 'mp3' or 'wav' file.
   ## 
   ##  Assumes one period in filename, at extension.
   ##
   ## ## COMMENTED ## check for now --- to try this
   ## script on various media files (movie as well as audio).
   ########################################################
   ...
   ########################################################
   ## Make the 'ogg' output filename.
   ########################################################

   FILENAMECROP=`echo "$FILENAME" | cut -d\. -f1`

   FILEOUT="${FILENAMECROP}.ogg"


   ########################################################
   ## Use 'ffmpeg' to make the 'ogg' file --- high-quality.
   ########################################################
