...
##
## Nautilus
## SCRIPT: 07_1movieFile_SCALE-resize_WxHpixels_ffmpeg-s_toMP4-H264-AAC.sh
##
## PURPOSE: Scale the frame-size of a movie file, down or up.
##
## METHOD:  Uses 'zenity --entry' to prompt for the new pixel size.
##
##          Uses 'ffmpeg' with '-s' (scale) --- and container, video codec,
##          and audio codec specified with '-f', '-vcodec', and '-acodec
##          respectively.
##
##          Shows the new, scaled movie file in a movie player.
##
##     NOTE on TESTING:
...
##
#############################################################################
## Started: 2013jun25 Based on FE Nautilus Scripts
##     07_1movieFile_SCALE-resize_WxHpixels_ffmpeg-s-Vdefault-Adefault_PRELIM.sh
##                 and
##     04b_1movieFile_CROP_Npixels_ffmpeg-crop-pad_toMP4-H264-AAC.sh 
## Changed: 2012
#############################################################################
...
###########################################################
## Prompt for Scale-to values --- width and height (pixels).
###########################################################

SCALE_WxH=""

SCALE_WxH=$(zenity --entry \
   --title "SCALE-TO values --- width and height (pixels)." \
   --text "\
Enter the size wanted for the video frames, width and height (pixels).

Examples: 320x240  or  640x480  or  800x600  or  1024x768  or  1920x1080

(You will probably want to choose values that preserve the aspect ratio
of the input movie file.)

The new file will have the string '__SCALEDmmmxnnn' in the filename." \
   --entry-text "640x480")
...
FILEOUT="${FILENAMECROP}_SCALED${SCALE_WxH}_h264-aac.mp4"
...
###################################################################
## Use 'ffmpeg' to make the scaled down/up movie file.
###################################################################
