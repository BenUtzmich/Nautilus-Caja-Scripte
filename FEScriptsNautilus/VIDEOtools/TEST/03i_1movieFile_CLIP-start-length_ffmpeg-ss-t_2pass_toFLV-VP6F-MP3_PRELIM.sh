...
##          (video-audio-container): video in VP6F format and
##          audio in MP3 format --- in an 'flv' container.
##             
...
##            'ffmpeg clip'  or  'ffmpeg ss flv vp6 mp3'  or ...
##
...
##           '0xx_oneMovie_CLIP_vidout-H264_audout-VORBIS_ffmpeg_inFLV.sh'.
## Changed: 2011dec13 Added '-copyts -async 1' to the 'ffmpeg' cmd.
## Changed: 2012may22 Changed script name in comments above and touched up
##                    the comments. Changed some indenting below.
## Changed: 2012jun18 Put '-ss' parm after '-i'. Added 'vp6-mp3' to FILEOUT.
############################################################################
...
FILEOUT="${FILENAMECROP}_START${STARTTIME}_DURATION${DURATION}_vp6-mp3.flv"
...
###########################################################
## DO WE NEED THE -ss and -t' parms on BOTH PASSES??
## CAN WE GO TO ONE PASS, IF NEED BE?
###########################################################

## FOR TEST: (show statements as they execute)
#   set -x

## Pass 1: (Could set the bitrate to '-b 750k', say,
##          to control the size/quality tradeoff.)

xterm -bg black -fg white -hold -geometry 90x48+100+100 -e \
    ffmpeg -i "$FILENAME" \
           -ss $STARTTIME -t $DURATION -f flv -an \
           -vcodec vp6f -threads 1 -pass 1 "$FILEOUT"
