...
##       this script specifies the 'mkv' (Matroska) container format.
##
##       NOTE on TESTING:
...
##            'ffmpeg clip'   or  'ffmpeg ss matroska h.264 vorbis'  or ...
##
...
##            This indicates that '-ss' and '-t' should be AFTER '-i'.
...
##                    Removed '-b' option (video bitrate) from 'ffmpeg'.
##                    Chgd '-f mkv' to '-f matroska'.
...
## Changed: 2012jun18 Put '-ss' parm after '-i'. Added 'h264-vorbis' to FILEOUT.
...
will be VORBIS.  The output container format will be MKV (Matroska).
------------------------------------------------------------------------
...
FILEOUT="${FILENAMECROP}_START${STARTTIME}_DURATION${DURATION}_h264-vorbis.mkv"
...
##     This seems to suggest that both '-ss' and '-t' should be AFTER '-i'.
################################################################################
...
xterm -hold -fg white -bg black -geometry 90x48+100+100 -e \
      ffmpeg -i "$FILENAME" \
            ...
            -crf 22 \
            -acodec libvorbis -ab 128k -ar 44100 -ac 1 \
            -threads 1 "$FILEOUT"
...
## NOTE: '-async 1' seems to be needed to get the audio clip to start with the
##       video clip --- or at least be pretty close to in sync.
##
## The vorbis audio (from an flv-mp3-flv file) seems nice and crisp-clear!
...
## ON VIDEO:
##
## Use '-b 200k'? Or use a larger value, like '-b 750k'?
##
