...
##
## Nautilus
## SCRIPT: 15a_1aniGIF_CONVERTto_FLV-SORENSON-noaudio_convert-coalesce-ffmpeg-an.sh
##
## PURPOSE: Converts a user-selected animated GIF file
##          to a '.flv' (flash container) movie file.
##          Uses Imagemagick 'convert' and the 'ffmpeg' utility.
##
## METHOD: Uses 'zenity --entry' to prompt for the frame rate at which to 
##         show the GIF file frames in the movie file.
##
##         Uses 'convert' with the '-coalesce' parm to convert the animated GIF
##         file into a sequence of '.png' files in the /tmp directory.
##
##         Uses 'ffmpeg' with parms including '-an' and '-r $FRAMERATE' and
##         '-i "/tmp/${FILENAMECROP}_frame%02d.png"' to make a '.flv' file.
##
