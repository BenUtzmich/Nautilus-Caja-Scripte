...
##          (video-audio-container): video in MPEG1 format and
##          and audio in MP3 format --- in an 'mpeg' (.mpg) container.
##             
...
##       this script specifies the 'mpeg' container format.
##
##       NOTE on TESTING:
...
##             'ffmpeg clip'   or  'ffmpeg ss mpeg mpeg1 mp2'  or ...
##
...
##            But other examples show '-ss' BEFORE the '-i'.
##
...
##           '0xx_oneMovie_CLIP_vidout-SVQ1_audout-MP3_inFLV_ffmpeg.sh'.
...
## Changed: 2012jun18 Put '-ss' parm after '-i'. Added 'mpeg1-mp2' to FILEOUT.
...
-----------------------------------------------------------------------
NOTE: The output video format will be MPEG1video. The output audio format
will be is MP2.  The output container format will be MPG (mpeg).
------------------------------------------------------------------------
...
FILEOUT="${FILENAMECROP}_START${STARTTIME}_DURATION${DURATION}_mpeg1-mp2.mpg"
...
#################################################################
## For some good info on the format of the 'ffmpeg' command
## for various output formats, see
...
xterm -hold -fg white -bg black -geometry 90x48+100+100 -e \
      ffmpeg -i "$FILENAME" \
            ...
            -copyts -async 1 \
            -vcodec mpeg1video \
            -acodec mp2 -ab 128k  -ar 22050 -ac 1 \
            -threads 1 "$FILEOUT"
...
##  '-async type#' needed?
##
## Add '-copyts'?   Is '-copyts' (copy time-stamps) needed?
