...
##
## Nautilus
## SCRIPT: 30e_1mkvCapFile_CONVERTto_MKV-H264-VORBIS_ffmpeg.sh
##
## PURPOSE: Using 'ffmpeg', this script converts an input movie file
##          to a '.mkv' container movie file.
##
...
##          a *LOSSY* H264 video stream and the input audio stream (possibly
...
##
##         Runs 'ffmpeg' with '-f mkv' and '-vcodec libx264' and
...
## is successfully convertible to an H264-VORBIS-MKV video file,
...
######################################
## Prepare the OUTPUT '.mkv' filename.
## (The suffix determines the container
## type that 'ffmpeg' creates.)
######################################

FILEOUT="/tmp/${USER}_MOVIEfileCONVERTEDto_LOSSY-h264-vorbis.mkv"
...
into lossy H264 video and lossy VORBIS audio in an MKV (Matroska)
format container in OUTPUT file
...
NEWSIZE=$(zenity --entry \
   --title "ENTER image SIZE for the new H264-VORBIS-MKV file to be made." \
   --text "\
Enter an image size for the MKV file
$FILEOUT
...
###########################################################
## Convert the input movie file into a lossy H264 video
## stream and lossy VORBIS audio stream in an MKV container.
###########################################################
## The parameters for this 'ffmpeg' command came
## FROM "HOWTO: Proper Screencasting on Linux" by verb3k,
## web URL above.
###########################################################
## We do run ffmpeg in a terminal window to watch progress,
## via messages from 'ffmpeg'.
###########################################################

xterm -bg black -fg white -hold -geometry 90x48+100+100 -e \
    ffmpeg -i "$FILENAME" -f mkv \
           -vcodec libx264 $SPARM \
