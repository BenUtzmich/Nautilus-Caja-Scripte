...
##
## Nautilus
## SCRIPT: 00_anyfile_XscreenANDpulseAUDIO_CAPTURElosslessTO_MKV-H264-PCM_ffmpeg_Qquit.sh
##
## PURPOSE: Using 'ffmpeg', this script captures Xwindow activity in
##          a video stream recorded in a 'LOSSLESS' H.264 movie format,
##          AND the 'ffmpeg' command captures audio from a pulseaudio
...
##          to setup for recording video and audio.
##
##          Uses 'zenity --entry' to prompt for number of threads to use in
##          doing the recording (to take advantage of multi-core machines).
##
##          Runs 'ffmpeg' with '-f alsa' and '-i pulse' and '-f x11grab'
##          and '-vcodec libx264' and '-acodec pcm_s16le' parms, along with
##          various other parms. Puts the video and audio streams in a
##          Matroska container (.mkv file).
##
...
##
##          This script shows the 'LOSSLESS' (relatively large) '.mkv' file
##          in a movie player.
##
############### NOTES ###################################################
##
...
############# MAINTENANCE HISTORY ###########################################
## Started: 2011may02
## Changed: 2011may06 Added zenity info-window.
## Changed: 2011may11 Added threads prompt.
...
## This tells 'ffmpeg' to capture a rectangle of 800×600 with an
...
FILEOUT="/tmp/${USER}_screencap_LOSSLESS-h264-pcm.mkv"
...
will use 'ffmpeg' to record the desktop
     Video Size   = $MOVIESIZE
     Video Offset = $OFFSET
into a LOSSLESS video stream ( vcodec = 'libx264' )
and  a LOSSLESS AUDIO stream ( acodec = 'pcm_s16le' )
put into a Matroska (mkv) container in output file
  $FILEOUT
...
recording.  Stop the recording by re-opening the
terminal window and typing 'q' (quit).  The terminal
...
and ffmpeg starts capturing the X11 screen ( Video Size =
