...
##
## Nautilus
## SCRIPT: 30a_1mkvCapFile_CONVERTto_MP4-H264-AAC_4youtube_ffmpeg.sh
##
## PURPOSE: Using 'ffmpeg', this script converts an input movie file
##          to a '.mp4' container movie file.
##
...
##          near LOSS-LESS) is converted to a *LOSSY* AAC audio stream.
##
...
###############################################################################
## Started: 2011may02
## Changed: 2011may06 Add zenity info-window.
...
## is successfully convertible to an H264-AAC-MP4 video file,
## via the following 'ffmpeg' command.
#######################################################################
...
FILEOUT="/tmp/${USER}_MOVIEfileCONVERTEDto_LOSSY-h264-aac.mp4"
...
into lossy H.264 video and lossy AAC audio in an MPEG4 container
in OUTPUT file
...
NEWSIZE=$(zenity --entry \
   --title "ENTER image SIZE for the new MP4-H264-AAC file to be made." \
   --text "\
Enter an image size for the MPEG4 file
$FILEOUT
...
The output file, if good, will be shown in a movie player." \
   --entry-text "640x480")
...
## stream and lossy *AAC* audio stream in an *mp4* (MPEG4) container.
########
