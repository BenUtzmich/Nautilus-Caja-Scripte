...
##
## Nautilus
## SCRIPT: 30d_1mkvCapFile_CONVERTto_WEBM-VP8-VORBIS_4html5_ffmpeg2pass_PRELIM.sh
##
## PURPOSE: Using 'ffmpeg', this script converts an input movie file
##          to a '.webm' container movie file.
##
...
##             a *LOSSY* VP8 video stream and the input audio stream (possibly
...
##
##         Runs 'ffmpeg' in 2 passes --- once with '-an -vcodec libvpx' parms, and
##         once with '-vcodec libvpx' and '-acodec libvorbis' parms.
##
##         No '-f' parm, for now, in this preliminary version.
##         (May need to install a post-2010 version of 'ffmpeg' to
##         create the webm container and the vp8 video stream.)
##
...
##         shows the '.webm' file in a movie player.
##
...
## is successfully convertible to an VP8-VORBIS-WEBM video file,
...
######################################
## Prepare the OUTPUT '.webm' filename.
## (The suffix determines the container
## type that 'ffmpeg' creates.)
######################################

FILEOUT="/tmp/${USER}_MOVIEfileCONVERTEDto_LOSSY-vp8-vorbis.webm"
...
into lossy VP8 video and lossy VORBIS audio in a WEBM container
in OUTPUT file
...
###########################################################
## Convert the input movie file into a lossy VP8 video
## stream and lossy VORBIS audio stream in a WEBM container.
## NOTE: This is a two-step process - 2 invocations of 'ffmpeg'.
###################################################################
## WebM is a new, high-quality, free/open format that can be played
## in modern web browsers that support the HTML5 <video> tag.
## Since libvpx does not have a constant quality mode yet, we do
## the encode in 2 passes.
## A '.webm' container is basically a subset of the Matroska container,
## aka '.mkv'.
###################################################################
## The parameters for these 2 'ffmpeg' commands came
...
## Pass 1: (Change the bitrate '-b 1000k' to control the size/quality
##          tradeoff.)

xterm -bg black -fg white -hold -geometry 90x48+100+100 -e \
    ffmpeg -i "$FILENAME" -an -vcodec libvpx $SPARM -b 1000k -pass 1 "$FILEOUT"
...
xterm -bg black -fg white -hold -geometry 90x48+150+150 -e \
    ffmpeg -i "$FILENAME" \
           -vcodec libvpx $SPARM -b 1000k \
           -acodec libvorbis -ab 96k -ar 22050 -ac 1 \
