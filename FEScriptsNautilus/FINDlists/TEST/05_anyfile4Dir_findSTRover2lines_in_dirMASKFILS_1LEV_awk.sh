...
##    
## Nautilus                        
## SCRIPT: 05_anyfile4Dir_findSTRover2lines_in_dirMASKFILS_1LEV_awk.sh
##                            
## PURPOSE: For all files, at ONE level under the current directory,
##          whose names match a given mask --- such as *.htm* or *.txt or *.sh,
##          this script lists the lines in those files that contain
##          a user-specified pair of strings that stretch over 2 lines.
##          The 'find' and 'awk' commands are used.
##
##         NOTE: I found this 'find-awk' technique useful when I had to
##               search about 100 '.htm' files to find pairs of lines of
##               the form
##                 </tr>
##                 </td>
...
##                 </tr>
##
## METHOD: This script uses 'zenity --entry' to prompt for a file mask.
##
##         Also, this script uses 'zenity --entry' to prompt for a pair of
##         strings --- STRING1 and STRING2.
##
##         This script uses an awk-based utility script named
##               '.findANDshow_strings1and2_over2linesINfile_awk.sh'
##         that is in the script directory with this script.
##
##         This awk-based utility script looks for STRING1 in a line and, if
##         found, looks for STRING2 in the next line. If a 2-line match is
##         found, the 2 lines are sent to stdout.
##
##         This script uses the 'find' command to limit the search to files
##         matching the user-specified file mask --- and to files at
##         the current directory level, no deeper (one level deep).
##
...
whose names match the mask '$MASK' --- ONE level
...
find . -maxdepth 1 -follow -type f -name "$MASK"  \
